...
class OldProductService
{
    ...
    public function replaceCharInDesc(UuidInterface $productId, string $oldChar, string $newChar) {
        $description = $this->oldProductDescriptionRepository->getOne($productId);

        $description->replace($oldChar, $newChar);

        $this->oldProductDescriptionRepository->save($description);
    }
